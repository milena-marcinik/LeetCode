...
class Solution:
    def maxIncreaseKeepingSkyline(self, grid: List[List[int]]) -> int:
        ...
        result_grid = 0
        for heights in grid:
            result_grid += sum(heights)
            for i in range(len(heights)):
                if len(bottom) < len(heights):
                    bottom.append(heights[i])
                else:
                    if heights[i] > bottom[i]:
                        bottom[i] = heights[i]

        new_grid = []
        result_new_grid = 0

        for new_heights in grid:
            for i in range(len(new_heights)):
                if bottom[i] > max(new_heights):
                    new_heights[i] = max(new_heights)
                else:
                    new_heights[i] = bottom[i]
            new_grid.append(new_heights)
            result_new_grid += sum(new_heights)

        return result_new_grid - result_grid
