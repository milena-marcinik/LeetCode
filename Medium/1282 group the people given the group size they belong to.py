...
class Solution:
    def groupThePeople(self, groupSizes: List[int]) -> List[List[int]]:
        ...
        for size in unique_sort_group_sizes:
            list_person_id.append([id for id in range(len(groupSizes)) if groupSizes[id] == size])

        result = []

        for element in list_person_id:
            if len(element) <= unique_sort_group_sizes[list_person_id.index(element)]:
                result.append(element)
            else:
                for i in range(0, len(element), unique_sort_group_sizes[list_person_id.index(element)]):
                    result.append(element[i:i + unique_sort_group_sizes[list_person_id.index(element)]])
