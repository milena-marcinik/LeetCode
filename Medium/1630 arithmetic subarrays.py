...
class Solution:
    def checkArithmeticSubarrays(self, nums: List[int], l: List[int], r: List[int]) -> List[bool]:
        range_queries = [(l[i], r[i]) for i in range(len(l))]
        queries = []
        for query in range_queries:
            new_nums = sorted(nums[query[0]:query[1] + 1])
            queries.append(new_nums)

        bool_list = []
        for query in queries:
            query_bool = []
            for i in range(len(query) - 1):
                if query[i + 1] - query[i] != query[1] - query[0]:
                    query_bool.append(False)
                else:
                    query_bool.append(True)
            bool_list.append(all(query_bool))
