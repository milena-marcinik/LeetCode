...
distinct. Your goal is to form arr by concatenating the arrays in pieces in any order.
...
class Solution:
    def canFormArray(self, arr: List[int], pieces: List[List[int]]) -> bool:
        for piece in pieces:
            if not set(piece) & set(arr):
                return False
            else:
                if len(piece) > 1:
                    for i in range(len(piece) - 1):
                        if arr.index(piece[i]) < arr.index(piece[i+1]):
                            return True
                        else:
                            return False
                else:
